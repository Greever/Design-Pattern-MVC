Das Observer Pattern gehört zu der Kategorie Verhaltensmuster. 
Mit ihm ist es möglich Änderungen von einem Objekt an ein anderes Objekt, welches von ersterem strukturell abhängig ist, weiterzugeben.
...
\section{Lösung: Das Observer Pattern}
	...
	\subsection{Variationen}
		...
		\subsubsection{Push-Modell}
			...
			\begin{itemize}
				\item[\Checkmark]{ Observer benötigt keine Informationen über Subjekt -> Starke Entkopplung!}
				\item[\XSolid]{Nicht jeder Observer benötigt alle/die selben Parameter!}
				\item[\XSolid]{Bei Erweiterung müssen alle Observer angepasst werden\\
		...
		\subsubsection{Pull-Modell}
			...
			\begin{itemize}
				\item[\Checkmark]{Jeder Observer holt sich per Getter nur die benötigten Informationen.}
				\item[\Checkmark]{Bei mehreren Subjekten: Eindeutig von welchem Subjekt!}
				\item[\XSolid]{Kann ineffizient werden, da Observer herausfinden muss was sich konkret verändert hat.}
			\end{itemize}
		...
		\begin{itemize}
			\item{Weiß das Subjekt von den Anforderungen der Observer -> Push-Modell}
			\item{Weiß das Subjekt nichts über die Observer -> Pull-Modell}
		\end{itemize}
